...
class LogisticRegression {
  constructor(features, labels, options) {
    ...
    this.costHistory = []; // cross entropy function
  ...
  gradientDescent(features, labels) {
    ...
    const differences = currentGuesses.sub(labels); // difference entre prediction et actual values
  ...
  test(testFeatures, testLabels) {
    const predictions = this.predict(testFeatures); // round to get 0 or 1 values
    testLabels = tf.tensor(testLabels).argMax(1);

    const incorrect = predictions
      .notEqual(testLabels)
      .sum()
      .get(); // get the value number inside the tensor
      ...
      const termTwo = this.labels
        ...
        .matMul(
          guesses
            .mul(-1)
            .add(1)
            .add(1e-7) // avoid getting a log(0)
            .log()
        );
